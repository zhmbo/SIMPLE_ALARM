...
- (void)viewDidLoad{
    ...
    [YQAnimationLayer createAnimationLayerWithString:@"闹小铃"
                                             andRect: CGRectMake(0, 100, self.view.frame.size.width, self.view.frame.size.width)
                                             andView:self.view
                                             andFont:[UIFont boldSystemFontOfSize:40]
                                      andStrokeColor:ALARMTHEMERED];
}
...
- (void)presentHomeVC
{
    UINavigationController *home_vc = [[self storyboard] instantiateViewControllerWithIdentifier:@"home_vc"];
    
    [self presentViewController:home_vc animated:YES completion:nil];
}
